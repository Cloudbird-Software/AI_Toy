...
- 工具链选型（固定）：Go = 单 module（go.mod）+ gofmt + errcheck + go vet + go test（testing/quick 属性测试）；BAML = `baml/` 类型化 prompt 契约 + baml-go 客户端；TS = pnpm + biome + vitest；Rust = cargo workspace + clippy + nextest；任务入口 = justfile；CI = GitHub Actions；门禁报告 = JSON 落库 `reports/`。
...
- LLM 评审纪律为全仓强制：仅用于不可断言维度；上线前置条件 = 与人工金标（每类错误 ≥15 例）Cohen's κ ≥ 0.61（κ≥0.80 才可进 CI 自主判定）；pairwise + 位置互换，不一致记 tie；judge 模型/温度版本锁定于 `configs/judge/`，prompt 契约锁定于 `baml/`。

## 0. 语言决策记录（IR #23，本节即立项记录）

决策依据：IR #23（Cloudbird-Software/AI_Toy issue，owner 已拍板）。语言选定后更换 = 重新立项（`languages.yaml`）；本次语言面修订于零业务上线时点执行，成本最低。

| 层 | 语言 | 依据 |
|---|---|---|
| application 层 | Go | 团队规范 default；Python 默认拒绝 |
| llm_prompt 层 | BAML | 类型化 prompt 契约，baml-go 客户端；prompt 源码收敛于 `baml/` |
| frontend | TypeScript | 保留 |
| 端侧推理 runtime | Rust | owner 豁免：ONNX/TFLite 绑定成熟、无 GC 抖动、实时音频事实标准；非 application 层（packages/native/） |

语言规则（全仓强制，编号与门禁规则同格式）：
- GO-1 `gofmt -l .` 零 diff（格式化不过 = lint 红）。
- GO-2 `errcheck ./...` 零未检错误。
- GO-3 可执行入口一律 `cmd/<name>/main.go`（工具与包内 CLI 均同）。
- GO-4 测试命令一律 `go test ./... -race -count=1`；属性测试用标准库 `testing/quick`（不引三方 property 库）。
- BAML-1 任何 prompt 变更必须过 golden test（prompt 契约 = 代码，走版本化评审，不经 `baml/` 生成的客户端不得内联裸调 LLM）。
...
├── justfile                         # §3.9
├── go.mod                            # Go module 根 §2.1（module github.com/Cloudbird-Software/AI_Toy）
├── package.json                     # §2.2
...
│   └── actions/{setup-go,paths,gate-report}/action.yml  # §5.9
├── docs/
...
├── tools/                            # 仓库特有工具链（§3，同属根 Go module）
│   ├── gaterunner/  cmd/gaterunner/ + internal/gaterunner/
│   ├── evalkit/     internal/evalkit/          # 纯库，无 CLI 入口
│   ├── judge/       cmd/toyjudge/ + internal/toyjudge/
│   ├── holdout/     cmd/holdoutctl/ + internal/holdoutctl/
│   ├── journeys/    cmd/journeys/ + internal/journeys/
│   ├── budgets/     cmd/budgets/ + internal/budgets/
│   ├── synthgen/    cmd/synthgen/ + internal/synthgen/
│   └── repoctl/     cmd/repoctl/ + internal/repoctl/
├── packages/
│   ├── go/                           # 每个 = Go 包（同属根 Go module）+ 独立 AGENTS.md
│   │   ├── turntaking/(T3) kws/(T4) speaker/(T5) imu/(T6) emotion/(T7) persona/(T8)
│   │   ├── safety/(T9) memory/(T10+T11) motion-map/(T12) tts/(T13) runtime-fsm/(T14go)
...
├── baml/                             # llm_prompt 层：类型化 prompt 契约（§0 语言决策）
│   ├── prompts/                      # *.baml 契约源（judge rubric 等，BAML-1 golden test 对象）
│   └── baml_client_go/               # baml-go 生成客户端（committed，Go 侧 import）
├── assets-packs/
│   ├── _template/                    # §11
...
│   ├── properties/                   # CI-1..CI-4 系统级状态机（*_test.go，testing/quick）
│   └── chaos/                        # 故障注入矩阵（*_test.go）
└── reports/
...
### FILE: go.mod
```go
module github.com/Cloudbird-Software/AI_Toy

go 1.23

require gopkg.in/yaml.v3 v3.0.1
```
（依赖纪律：标准库优先；三方依赖唯一白名单 = `gopkg.in/yaml.v3`（configs/*.yaml 解析）；`baml/baml_client_go/` 为生成物随仓 committed，不作外部依赖引入。lint 面见 §3.9：gofmt 零 diff（GO-1）+ errcheck（GO-2）+ go vet。）
...
## 3. 仓库特有工具（tools/*，同属根 Go module，go test 自测）

每个工具包结构（Go 标准，GO-3）：`cmd/<name>/main.go`（CLI 入口）+ `internal/<pkg>/`（实现）+ `internal/<pkg>/…_test.go`（测试与实现共置）；纯库工具（evalkit）无 cmd/。执行者实现到「CLI 契约可跑、自测通过」即可，内部实现自主。本节固定的是 CLI、退出码、产物 schema——这些是 CI 与其它工具的依赖面，不得偏离。

### 3.1 tools/gaterunner（门禁运行器——本仓核心）

职责：读 `configs/gates/<asset>.yaml` 阈值 → 调度对应 go test 注册标记的测试 → 收集指标 → 按统计规则判定 → 产出 JSON 报告。阈值永不硬编码在测试里。

CLI：
```
gaterunner collect                      # 解析 go test 收集（注册标记，见 §6 开发循环）+ configs/gates → 打印/写出断言登记表
...
函数契约（Go 惯例签名）：`zero_fail_n(q float64) int`、`poisson_upper95(k int, n float64) float64`、`binom_upper95(k, n int) float64`、`wilson(k, n int) (lo, hi float64)`、`paired_bootstrap(a, b []float64, iters int) (diff float64, ci [2]float64)`（iters 默认 10000）、`permutation_p(a, b []float64) float64`、`eer(scores []float64, labels []bool) (eer float64, misses, falseAlarms int)`、`noise_band(values []float64) (mean, sigma float64)`、`cohens_kappa(r1, r2 []int) float64`。规则：不做近似捷径；所有 CI 返回 95% 双侧；自带对照用例（泊松 3/N、n≥59 等作单测）。

### 3.3 tools/judge（LLM-as-verifier 评审机，Go 包名 `toyjudge`）
...
强制行为：judge 模型/温度从 `configs/judge/model.yaml` 锁定读取；prompt 一律经 `baml/` 生成的客户端（`baml_client_go`）调用，报告记录其哈希（BAML-1：prompt 变更必须过 golden test）；AB/BA 各评一次不一致记 tie；不用被评模型同族 judge；rubric 为三级量表（1/2/3）+ 每档锚定样例，禁 1–5 打分量表；高风险 rubric（9a）双 judge。

### 3.4 tools/holdout（密封 holdout 客户端，Go 包名 `holdoutctl`）
...
CLI：`journeys run --set golden --seeds 3 --driver packages/go/user-sim`。剧本 = `tests/golden-journeys/*.yaml`（schema：`id, tier(core|variant), persona{age,patience}, steps[...], inject{interrupts, safety_events}, assertions[]`）。断言与 §10 system.md 的旅程门禁一致；产出逐旅程完成率/延迟/安全事件/记忆命中。
...
jobs:
  changes:
    ...
    outputs: { go: ${{ steps.f.outputs.go }}, ts: ${{ steps.f.outputs.ts }}, rs: ${{ steps.f.outputs.rs }},
    ...
    steps:
      ...
      - uses: dorny/paths-filter@v3
        ...
        with:
          filters: |
            go: [tools/**, packages/go/**, baml/**, tests/**, configs/**]
            ts: [packages/ts/**]
            rs: [packages/native/**]
      ...
      - id: g
        run: |
          # 改动路径 → 受影响资产 → 附加跑组合级
          echo "assets=$(go run ./tools/repoctl/cmd/repoctl affected --base origin/main)" >> "$GITHUB_OUTPUT"
  go:
    needs: changes
    ...
    steps:
      ...
      - run: test -z "$(gofmt -l .)" && go vet ./... && errcheck ./...   # GO-1 + GO-2
      - run: go test ./... -race -count=1 -short -timeout=600s           # GO-4（排 slow/journey）
  ...
  gates:
    ...
    steps:
      - uses: actions/checkout@v4
      - uses: ./.github/actions/setup-go
      - run: go run ./tools/gaterunner/cmd/gaterunner verify-configs
      - run: |
          for a in $(echo '${{ needs.changes.outputs.assets }}' | jq -r '.[]'); do
            go run ./tools/gaterunner/cmd/gaterunner run --asset "$a" --level all --suite ci --report "reports/gates/$a.json"
          done
  ...
  combo-smoke:                                  # 核心 10 旅程 + 故障矩阵前 3 行
    ...
    steps:
      - uses: actions/checkout@v4
      - uses: ./.github/actions/setup-go
      - run: go run ./tools/journeys/cmd/journeys run --set core10 --seeds 1
      - run: go test ./tests/chaos/... -race -count=1 -run "TestLLMOutage|TestTTSTimeout|TestMemoryUnwritable"
  meta:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
      - uses: ./.github/actions/setup-go
      - run: go run ./tools/repoctl/cmd/repoctl coverage && go run ./tools/repoctl/cmd/repoctl agents-md check && go run ./tools/repoctl/cmd/repoctl forbidden-refs && go run ./tools/repoctl/cmd/repoctl exemption audit
...
```
Required checks（分支保护）：`go / ts / rs`（按需）、`gates`、`combo-smoke`、`meta`、`gates-cfg-guard`。
...
jobs:
  g0-sweep:        { runs-on: [self-hosted, gpu], steps: [{uses: actions/checkout@v4},{uses: ./.github/actions/setup-go},
                    {run: for a in T1 T2 T3 T4 T5 T6 T7 T8 T9 T10 T12 T13 T14 T15 T16 T20; do go run ./tools/gaterunner/cmd/gaterunner run --asset $a --level g0 --suite nightly --report reports/gates/$a.json || exit 10; done}] }
  synth-holdout:   { runs-on: [self-hosted, gpu], steps: [...,{run: go run ./tools/gaterunner/cmd/gaterunner run --suite nightly --dataset-track synth-holdout}] }
  golden-journeys: { runs-on: [self-hosted, gpu], steps: [...,{run: go run ./tools/journeys/cmd/journeys run --set golden --seeds 3 --out reports/nightly/journeys.json}] }
  latency:         { runs-on: [self-hosted, gpu], steps: [...,{run: go run ./tools/budgets/cmd/budgets check --report reports/nightly/latency.json}] }
  chaos:           { runs-on: [self-hosted, gpu], steps: [...,{run: go test ./tests/chaos/... -race -count=1}] }
  no-leak:         { runs-on: ubuntu-latest, steps: [...,{run: go test ./packages/go/data-flywheel/... -race -count=1 -run LeakAudit}] }   # minhash 交叉 + 拓扑断言
  meta:            { runs-on: ubuntu-latest, steps: [...,{run: just coverage}] }
  d4-snapshot:     { runs-on: [self-hosted, gpu], steps: [...,{run: go run ./packages/go/router/cmd/cost_curve --snapshot reports/nightly/d4.json}] }
  report:          { needs: [g0-sweep,synth-holdout,golden-journeys,latency,chaos,no-leak,meta,d4-snapshot],
                     runs-on: ubuntu-latest, steps: [...,{run: go run ./tools/repoctl/cmd/repoctl nightly-index}] }   # 汇总单页索引 + 失败摘要
```
...
jobs:
  ...
  judge-recal:   { runs-on: [self-hosted, gpu], steps: [...,{run: for r in 7a 8a 9a 13a; do go run ./tools/judge/cmd/toyjudge calibrate --rubric $r --gold configs/judge/gold/$r.jsonl || exit 20; done}] }
  drift-report:  { runs-on: ubuntu-latest, steps: [...,{run: go run ./tools/repoctl/cmd/repoctl drift --weeks 4 --out reports/nightly/drift.md}] }
  ml-self-audit: { runs-on: ubuntu-latest, steps: [...,{run: go run ./tools/repoctl/cmd/repoctl ml-test-score --checklist docs/runbooks/ml-test-score.yaml}] }
  data-audit:    { runs-on: ubuntu-latest, steps: [...,{run: go run ./tools/holdout/cmd/holdoutctl audit && go run ./tools/repoctl/cmd/repoctl license-ledger}] }
```
...
jobs:
  sealed-eval:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: go run ./tools/holdout/cmd/holdoutctl verify-seal
      - run: go run ./tools/holdout/cmd/holdoutctl eval --suite real --out reports/nightly/real-holdout.json   # 仅聚合输出，n<5 切片抑制
      - run: go run ./tools/holdout/cmd/holdoutctl audit
...
jobs:
  ...
  canary:      { needs: g0-all, runs-on: [self-hosted, gpu], steps: [...,{run: go run ./tools/holdout/cmd/holdoutctl eval --suite canary}] }   # canary 掉分=exit 10
  packs:       { needs: g0-all, runs-on: ubuntu-latest, steps: [...,{run: go test ./packages/go/packs/... -race -count=1 -run TestBuildAndIsolate},
                {run: for p in assets-packs/*/; do go run ./packages/go/packs/cmd/packs build "$p"; done}] }
  sbom:        { needs: [g0-all,packs], runs-on: ubuntu-latest, steps: [...,{run: go run ./tools/repoctl/cmd/repoctl sbom --out dist/sbom.spdx}] }
```
...
`.github/actions/setup-go/action.yml`：checkout → install Go 1.23 → `go mod download` → 恢复 go module 缓存。`.github/actions/gate-report/action.yml`：上传 `reports/gates/*.json` artifact + PR 评论渲染 `gaterunner summary`。
...
5. Dependabot：gomod/pnpm/cargo/github-actions 周更。
...
### 7.1 GEN：全部包骨架
对 `packages/go/*` 每包创建：`internal/…`（实现 + `…_test.go` 共置，GO-4）、`gates_test.go`（空骨架但 gaterunner 注册标记注释齐）、`cmd/<pkg>/main.go`（仅带 CLI 的包，GO-3；test 依赖声明 = 同 module 直接 import `evalkit`/`gaterunner`）、`AGENTS.md`、`README.md`（3 行：做什么/验收文档路径/just 命令）。`packages/ts/*`、`packages/native/*` 同理（vitest / nextest）。

### 7.2 FILE 模板：packages/go/<pkg>/AGENTS.md（通用模板，`{{}}` 由 §7.3 表填充）
```markdown
# AGENTS.md — {{资产名}}（{{TID}}）
...
## 本地命令
just gate {{TID}} all ；go test ./packages/go/{{pkg}} -run Property -count=1
## 本地必绿再提 PR
{{该资产 G0/G1 的 id 列表}}
## 数据依赖
...
| runtime-fsm | T14 离线运行时(go 侧) | G0：编造≤5%；切换 0 脏输出/0 记忆损失；4 档安全不降级。G1：离线旅程≥80%；冷启动≤3s | 200「端侧必不会」问题集；黄金旅程 | FSM 表驱动穷举必做；TLA+ 触发条件见 docs/gates/assets/T14.md |
...
### 7.4 完整示例一：FILE packages/go/kws/AGENTS.md
```markdown
# AGENTS.md — 唤醒词（T4）
...
### 7.5 完整示例二：FILE packages/go/safety/AGENTS.md
```markdown
...
### 8.2 tests/properties/（组合不变量，Go 快速检查 testing/quick）
- `ci1_degrade_monotonic_test.go`：CI-1 任意故障序列 → 全局能力集 ⊆ 各组件档位能力交集；安全配置=最严格者；降档安全水位单调不降
- `ci2_identity_binding_test.go`：CI-2 任何输出可回溯唯一身份；T5 拒判瞬间记忆通道转只读缓存
- `ci3_budget_conservation_test.go`：CI-3 Σ分段 P95−并行重叠 ≤1500；>2σ 劣化无划拨 → 红
- `ci4_failure_grace_test.go`：CI-4 依赖失效 → 行为 ∈ 预定义降级集；无响应 >10s 不道歉 = 违反

### 8.3 tests/chaos/（故障注入矩阵，8 个 `*_test.go` 一文件一测）
| 注入 | 期望 | 恢复 | 门禁 |
|---|---|---|---|
...
属性测试四性质族（Go testing/quick）：有界性（状态 ∈ 合法域：情绪∈[0,1]、亲密度∈[0,100]、占空比≤上限）；单调性（SNR↓唤醒率不升；攻击强度↑判定不宽松；θ↑命中率与误命中同升）；不变性（增益/时移/codec/同义改写不改判定）；确定性（同输入同输出或同分布，回放可复现）。蜕变测试（paraphrase 不变性等）按需复用；API 契约用 Schemathesis。
形式化验证只投两处：T14 降级 FSM、T10 记忆生命周期 FSM。两步法：表驱动穷举（必做：状态×事件全表、无死锁、全可达、吸收态正确、隔离不可达）→ TLA+/TLC（可选，触发条件：表驱动发现竞态 bug 或同区连续两次回归）。Go 侧用 testing/quick 做运行时镜像。
...
包：packages/go/turntaking　阈值：configs/gates/T3.yaml　验收层：L1+L2+L5
...
```markdown
# T4 唤醒词（KWS）
包：packages/go/kws　阈值：configs/gates/T4.yaml　验收层：L1+L2+L5
...
```markdown
# T5 声纹与家庭成员识别
包：packages/go/speaker　阈值：configs/gates/T5.yaml　验收层：L1+L2+L5
...
```markdown
# T7 情绪引擎（检测+状态动力学+恢复）
包：packages/go/emotion　阈值：configs/gates/T7.yaml　验收层：L1+L2+L4+L5（五层全用）
...
```markdown
# T12 情绪→表情/动作映射（表现层）
包：packages/go/motion-map　阈值：configs/gates/T12.yaml　验收层：L1+L2+L4+L5
...
包：packages/go/tts　阈值：configs/gates/T13.yaml　验收层：L1+L2+L4+L5
...
包：packages/go/eval-platform　阈值：configs/gates/T1.yaml　验收层：L1+L2（元层）
...
技术路径：A promptfoo 核心+自建 CI 适配（MIT，声明式 YAML 评测+内建 LLM 断言统计；T8/T9/T10 断言层默认执行器）；B DeepEval（Apache-2.0，xUnit 风格单测化；A 管探索对比、B 管回归门禁，不互斥）；C 薄自建（调度+落库+看板；语音/时序/真机断言本不在 LLM 框架内，无论 A/B 如何选 C 必须有——汇成一张门禁报表的胶水层）。落地绑定：本仓 gaterunner 即 C 路径的实体；A/B 作执行后端按资产接入（外部后端经子进程/服务调用，与 application 层语言解耦）。
门禁（元门禁）：
| id | BI | 断言 | 口径与样本量 | 阈值 | 级 |
|---|---|---|---|---|---|
...
包：packages/go/data-flywheel　阈值：configs/gates/T2.yaml　验收层：L1+L2+L5
...
[5] testing/quick: property-based testing in the Go standard library — pkg.go.dev/testing/quick
...
go run ./tools/gaterunner/cmd/gaterunner collect | wc -l                   # ≥ 70 条断言登记（≈70 G0 全量）
just gate T4 all                                    # 任一资产样板跑通（阈值红是正常——实现未开始；exit 2 配置错才是本阶段失败）
ls docs/gates/assets/ | wc -l                       # = 16
ls packages/go/*/AGENTS.md | wc -l                  # = 17；全仓 = 17 go + 2 ts + 2 native + 1 根 = 22（§7.3 表覆盖 19，ts 两包按 §7.1 同理生成）
